#include "LEDSlowerWindow.h"

static constexpr const char* TAG = "LEDSlowerWindow";

LEDSlowerWindow::LEDSlowerWindow(lv_obj_t* parent, const LEDSlowerData* params) : HomeWindow(parent, TITLE){
	buildUI(*params);
}

void LEDSlowerWindow::buildUI(const LEDSlowerData data){
	if(data.missing){
		// Image
		...
		// GIF
		gif = new LVGIF(innerContent, theme->getAsset(Asset::LedSlower));
...
void LEDSlowerWindow::onDismiss(){
