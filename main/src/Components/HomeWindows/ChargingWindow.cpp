...
ChargingWindow::ChargingWindow(lv_obj_t* parent, const BatteryData* params) : HomeWindow(parent, TITLE){
	buildUI(*params);
}

void ChargingWindow::buildUI(const BatteryData data){
	// Image
	lv_obj_t* img = lv_image_create(innerContent);
	lv_image_set_src(img, theme->getAsset(Asset::BatteryCharging));
	lv_obj_set_style_pad_ver(img, 8, 0);

	// Sentence label
	lv_obj_t* sentenceLabel = lv_label_create(innerContent);
	lv_label_set_text(sentenceLabel, Phrases::map(Phrase::BatteryCharging, data.id).c_str());
