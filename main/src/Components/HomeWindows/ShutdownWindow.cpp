...
ShutdownWindow::ShutdownWindow(lv_obj_t* parent, const ShutdownData* params) : HomeWindow(parent, TITLE){
	buildUI(*params);
}

void ShutdownWindow::buildUI(const ShutdownData data){
	const Asset asset = (data.reason == ShutdownData::ShutdownReason::Battery)
		                    ? Asset::BatteryLow
		                    : Asset::LedOff;
