#include "LEDTurnOnWindow.h"

LEDTurnOnWindow::LEDTurnOnWindow(lv_obj_t* parent, const LEDTurnOnData* params) : HomeWindow(parent, TITLE){
	buildUI(*params);
}

void LEDTurnOnWindow::buildUI(const LEDTurnOnData data){
	// Image
	lv_obj_t* img = lv_image_create(innerContent);
	lv_image_set_src(img, theme->getAsset(data.missing ? Asset::ModuleUnplugged : Asset::LedOn));
